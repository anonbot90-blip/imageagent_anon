"""
Planner Training Dataset - Standardized Weighted Text-Only Version

Dataset for fine-tuning Qwen3-VL using standardized trajectory-level weights.
...
class PlannerDatasetSWTrajectoryTextComplex(Dataset):
    """
    Standardized weighted dataset for action planner training (text-only).
    ...
    def __init__(
        ...
        """
        Initialize standardized weighted text-only dataset.
        ...
        print(f"Loaded {len(self.data)} training samples (standardized weighted trajectory-based, text-only mode)")
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        """Get training sample with standardized weight."""
...
def create_dataloaders(
    ...
    """
    Create train and validation dataloaders (standardized weighted text-only).
    ...
    # Create full dataset
    full_dataset = PlannerDatasetSWTrajectoryTextComplex(
        data_path=data_path,
        processor=processor,
