"""
Train Action Planner - Reward-Weighted Cached Embeddings Mode

Fine-tune Qwen3-VL using reward-weighted samples with pre-computed vision embeddings.
High-quality samples get higher weights during training (3× faster than full vision).
"""
...
from training.planner_training.planner_dataset_rw_cached import create_dataloaders
...
def load_config(config_path: str = None):
    ...
    if config_path is None:
        config_path = SCRIPT_DIR / "configs_complex_8b" / "planner_config_rw_trajectory_cached.yaml"
...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs_complex_8b/planner_config_rw_trajectory_cached.yaml",
        help="Path to config file (default: configs_complex_8b/planner_config_rw_trajectory_cached.yaml)"
    )
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    print("=" * 60)
    print("🧠 Action Planner Training - Reward-Weighted Cached Mode")
    print("=" * 60)
    print()
    ...
    # Start training
    print("\n" + "=" * 60)
    print("🚀 Starting Reward-Weighted Training (Cached Embeddings)")
    print("=" * 60)
    print()
