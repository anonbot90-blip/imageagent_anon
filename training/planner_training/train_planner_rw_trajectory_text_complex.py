"""
Train Action Planner - Reward-Weighted Text-Only Mode

Fine-tune Qwen3-VL using reward-weighted samples (text-only, no images).
High-quality samples get higher weights during training.
"""
...
from training.planner_training.planner_dataset_rw_text import create_dataloaders
...
def load_config(config_path: str = None):
    ...
    if config_path is None:
        config_path = SCRIPT_DIR / "configs_complex_8b" / "planner_config_rw_trajectory_text.yaml"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train Qwen3-VL Action Planner (Reward-Weighted Text-Only)")
    parser.add_argument(
        "--config",
        type=str,
        default="configs_complex_8b/planner_config_rw_trajectory_text.yaml",
        help="Path to config file (default: configs_complex_8b/planner_config_rw_trajectory_text.yaml)"
    )
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    print("=" * 60)
    print("🧠 Action Planner Training - Reward-Weighted Text-Only Mode")
    print("=" * 60)
    print()
    ...
    print("⚡ Reward-weighted text-only mode: High-quality samples emphasized")
    ...
    # Uses WeightedLossTrainer to apply sample weights to loss
