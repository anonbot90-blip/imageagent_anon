"""
Train Action Planner - Standardized Weighted Cached Embeddings Mode

Fine-tune Qwen3-VL using standardized trajectory-level weights with pre-computed vision embeddings.
Samples weighted by trajectory z-scores (standardized rewards) (3× faster than full vision).
"""
...
from training.planner_training.planner_dataset_sw_trajectory_cached_complex import create_dataloaders
...
def load_config(config_path: str = None):
    ...
    if config_path is None:
        config_path = SCRIPT_DIR / "configs_complex_8b" / "planner_config_sw_trajectory_cached.yaml"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train Qwen3-VL Action Planner (Standardized Weighted Cached)")
    parser.add_argument(
        "--config",
        type=str,
        default="configs_complex_8b/planner_config_sw_trajectory_cached.yaml",
        help="Path to config file (default: planner_config_sw_trajectory_cached.yaml)"
    )
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    print("=" * 60)
    print("🧠 Action Planner Training - Standardized Weighted Cached Mode")
    print("=" * 60)
    print()
    ...
    # Create dataloaders with cached embeddings
    print("\nCreating dataloaders with cached embeddings...")
    ...
    # Start training
    print("\n" + "=" * 60)
    print("🚀 Starting Standardized Weighted Training (Cached Embeddings)")
    print("=" * 60)
    print()
