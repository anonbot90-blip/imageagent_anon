"""
Planner Training Dataset - Reward-Weighted Text-Only Version

Dataset for fine-tuning Qwen3-VL using reward-weighted samples.
...
class PlannerDatasetRWTrajectoryTextComplex(Dataset):
    """
    Reward-weighted dataset for action planner training (text-only).
    ...
    def __init__(
        ...
        """
        Initialize reward-weighted text-only dataset.
        ...
        print(f"Loaded {len(self.data)} training samples (reward-weighted trajectory-based, text-only mode)")
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        """Get training sample with weight."""
        ...
        # Create conversation format for training (text-only)
        ...
        # Get sample weight (computed but not used in loss - RW benefit is from data selection)
        weight = self.weights[idx]
        
        # Return text-only inputs with sample weight
        result = {
            "input_ids": input_ids,
...
def create_dataloaders(
    ...
    """
    Create train and validation dataloaders (reward-weighted text-only).
    ...
    # Create full dataset
    full_dataset = PlannerDatasetRWText(
        data_path=data_path,
        processor=processor,
    ...
    def collate_fn(batch):
        """Collate batch with sample weights."""
