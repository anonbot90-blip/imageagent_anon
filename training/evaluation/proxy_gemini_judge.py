#!/usr/bin/env python3
"""
Gemini 2.5 Flash Judge — uses internal LLM proxy (OpenAI-compatible API).
Same prompts as GeminiJudge for fair comparison.
Reads LLM_PROXY_KEY and LLM_PROXY_ENDPOINT env vars.

Note: gemini-2.5-flash is a thinking model — needs max_tokens >= 500.
"""
...
def _call_proxy(client, model, messages, max_tokens=2048, max_retries=3) -> dict:
    """Call proxy and parse JSON. Uses high max_tokens for thinking model."""
...
class ProxyGeminiJudge(BaseJudge):
    """Image quality judge using Gemini 2.5 Flash via LLM proxy."""

    def __init__(self):
        self.client = _get_client()
        self.model = os.getenv("LLM_PROXY_GEMINI_MODEL", "gemini-2.5-flash")
        print(f"✅ ProxyGeminiJudge initialized (model: {self.model})")
...
{{
  ...
  "reasoning": "<brief 1-2 sentence explanation>"
        ...
        try:
            return _call_proxy(self.client, self.model, messages, max_tokens=2048, max_retries=max_retries)
        except Exception as e:
    ...
    def judge_action_plan(self, original_image, user_prompt, predicted_plan, teacher_plan=None, max_retries=3):
        return ProxyGeminiActionJudge._do_judge(self, original_image, user_prompt, predicted_plan, max_retries)


class ProxyGeminiActionJudge(BaseJudge):
    """Action plan quality judge using Gemini 2.5 Flash via LLM proxy."""

    def __init__(self):
        self.client = _get_client()
        self.model = os.getenv("LLM_PROXY_GEMINI_MODEL", "gemini-2.5-flash")
        print(f"✅ ProxyGeminiActionJudge initialized (model: {self.model})")
    ...
    def _do_judge(self_obj, original_image, user_prompt, predicted_plan, max_retries):
        ...
        formatted_plan = ProxyGeminiActionJudge._format_action_plan(predicted_plan)
...
{{
  ...
  "explanation": "<brief 2-3 sentence explanation focusing on key strengths/weaknesses of both actions AND reasoning>"
        ...
        try:
            scores = _call_proxy(self_obj.client, self_obj.model, messages, max_tokens=4096, max_retries=max_retries)
    ...
    def judge_single_edit(self, original_image, generated_image, instruction, max_retries=3):
        return ProxyGeminiJudge().judge_single_edit(original_image, generated_image, instruction, max_retries)
