#!/usr/bin/env python3
"""
Batch Image Generator using HiDream-I1
Efficiently generates multiple images by loading the model once
"""
...
class BatchImageGenerator:
    """Batch image generator that loads model once and generates multiple images"""
    ...
    def __init__(self, prompts_file: str, output_base_dir: str, gpu_id: str = None):
        ...
        print(f"🎨 Batch Image Generator initialized")
    ...
    def _load_prompts(self) -> List[Dict]:
        ...
        Handles both old format (flat) and new format (with 'generation' nested object)
        """
        ...
        for prompt in prompts:
            # Check if this is the new format with 'generation' and 'edit' nested objects
            if 'generation' in prompt:
                # Extract generation data and merge with top-level metadata
                normalized = {
                    'id': prompt.get('id'),
                    'name': prompt.get('name', ''),
                    'category': prompt.get('category', 'unknown'),  # V2: Required for transformation mapper
                    **prompt['generation']  # Merge generation fields (text, style/theme, model, resolution)
                }
                
                # Normalize the 'style' or 'theme' key to 'style' for consistency
                if 'theme' in prompt['generation']:
                    normalized['style'] = prompt['generation']['theme']
                elif 'style' not in normalized:
                    # Fallback if neither style nor theme is present
                    normalized['style'] = f"image_{prompt.get('id', 'unknown')}"
                
                # Also store edit data for reference (pipeline will use it)
                if 'edit' in prompt:
                    normalized['edit_info'] = prompt['edit']
            else:
                # Old format - use as is
                normalized = prompt
                # Ensure 'style' key exists
                if 'style' not in normalized:
                    normalized['style'] = f"image_{normalized.get('id', 'unknown')}"
    ...
    def generate_batch(self, num_images: int = 5) -> str:
        ...
        """
        print("=" * 60)
        print(f"🚀 Starting batch generation of {num_images} images")
        print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Batch Image Generator using HiDream-I1')
    parser.add_argument('--prompts', '-p', required=True, help='Path to prompts.json file')
    ...
    if args.output_dir is None:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        args.output_dir = f"generation_output_{timestamp}"
