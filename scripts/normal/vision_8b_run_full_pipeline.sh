...
# Full Vision Training Pipeline (Trajectory-Based with Cached Embeddings)
...
#   scripts/evaluation/normal/start_eval_all_planner_vision_trajectory.sh
...
CHECKPOINT_BASE="$PROJECT_ROOT/checkpoints/normal/cot_8b_trajectory/vision"
...
print_header "Step 1/7: Verify Trajectory Split"

if [ ! -f "$TRAJECTORY_BASE/test_samples_cot_8b.txt" ]; then
...
TRAIN_COUNT=$(wc -l < "$TRAJECTORY_BASE/train_samples_cot_8b.txt")
TEST_COUNT=$(wc -l < "$TRAJECTORY_BASE/test_samples_cot_8b.txt")
...
print_header "Step 2/7: Verify Training Data"
...
if [ ! -f "$TRAJECTORY_DIR/standard_vision/planner_training_data.json" ]; then
    ...
    echo -e "${GREEN}✓ Vision training data already exists${NC}"
fi
...
print_header "Step 3/7: Precompute Vision Embeddings (if needed)"
...
WANDB_MODE=offline CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun \
    ...
    --config configs_8b/planner_config_standard_trajectory_cached.yaml
...
WANDB_MODE=offline CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun \
    ...
    --config configs_8b/planner_config_rl_trajectory_cached.yaml
...
WANDB_MODE=offline CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun \
    ...
    --config configs_8b/planner_config_rw_trajectory_cached.yaml
...
WANDB_MODE=offline CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun \
    ...
    --config configs_8b/planner_config_dpo_trajectory_cached.yaml

echo -e "${GREEN}✓ DPO vision 8B model trained${NC}"
sleep 10

# SW Training
print_header "Step 8/8: Train SW Vision Model - 8B (port 29518)"

WANDB_MODE=offline CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun \
    --nproc_per_node=8 \
    --master_port=29518 \
    train_planner_sw_trajectory_cached.py \
    --config configs_8b/planner_config_sw_trajectory_cached.yaml
...
print_header "📈 Evaluate Vision Models (NORMAL 10K - 8B)"
...
# Use unified config-based evaluation for normal vision 8B
bash scripts/evaluation/run_parallel_evaluation.sh \
    --config scripts/evaluation/configs/normal_vision_8b.yaml
...
echo -e "${CYAN}   Evaluation: evaluation_results/vision_parallel_cot_8b_trajectory/${NC}"
echo ""

echo -e "${YELLOW}💡 Next steps:${NC}"
echo -e "${YELLOW}   - Review evaluation results in evaluation_results/vision_parallel_cot_8b_trajectory/${NC}"
echo -e "${YELLOW}   - Compare with sample-based baseline (evaluation_results/vision_parallel_cot_8b/)${NC}"
echo ""
