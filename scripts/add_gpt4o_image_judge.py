#!/usr/bin/env python3
"""
Add GPT-4o image judge scores to GPT-4o's evaluation results.
"""
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python add_gpt4o_image_judge.py <dataset_dir>")
        print("")
        print("Example:")
        print("  python add_gpt4o_image_judge.py evaluation_results/text_parallel_cot_8b_trajectory/gpt4o")
        sys.exit(1)
    ...
    for i, sample in enumerate(detailed_results, 1):
        ...
        print(f"  [{i}/{len(detailed_results)}] Processing {sample_id}...", end=" ", flush=True)
