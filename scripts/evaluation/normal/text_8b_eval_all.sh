#!/bin/bash
# Parallel evaluation for TRAJECTORY-BASED 8B baseline, standard, RL, RW, DPO, SW, and GPT-4o text-only models - 4B
# TEXT-ONLY TRAJECTORY EXPERIMENT (8B): Runs 8 models in PARALLEL (7 on GPUs + 1 via API)
# Ensures ALL models evaluate the SAME samples for fair comparison

set -e  # Exit on error

# ════════════════════════════════════════════════════════════════════════════════
# CONFIGURATION - 8B MODEL
# ════════════════════════════════════════════════════════════════════════════════
...
TEST_SAMPLES_FILE="$PROJECT_ROOT/training_data/cot_8b_trajectory/test_samples_cot_8b_resume.txt"
OUTPUT_BASE="$PROJECT_ROOT/evaluation_results/normal/text_parallel_cot_8b_trajectory"
NUM_SAMPLES=95  # Resume: 95 remaining samples (first 104 already done)
EVAL_CHECKPOINT=20  # Generate tables after every N samples (0 = only at end)
...
# Model checkpoints - TRAJECTORY-BASED 8B
BASELINE_CHECKPOINT="Qwen/Qwen3-VL-8B-Instruct"
EDIT_ONLY_CHECKPOINT="none"  # E baseline is inference-only, no checkpoint
STANDARD_TEXT_CHECKPOINT="$PROJECT_ROOT/checkpoints/normal/cot_8b_trajectory/standard/final"
RL_TEXT_CHECKPOINT="$PROJECT_ROOT/checkpoints/normal/cot_8b_trajectory/rl/final"
RW_TEXT_CHECKPOINT="$PROJECT_ROOT/checkpoints/normal/cot_8b_trajectory/rw/final"
DPO_TEXT_CHECKPOINT="$PROJECT_ROOT/checkpoints/normal/cot_8b_trajectory/dpo/final"
SW_TEXT_CHECKPOINT="$PROJECT_ROOT/checkpoints/normal/cot_8b_trajectory/sw/final"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            cat << EOF
Usage: bash scripts/evaluation/start_eval_all_planner_text_trajectory_8b.sh [OPTIONS]

TRAJECTORY-BASED 8B TEXT-ONLY EXPERIMENT (PARALLEL): Evaluates baseline, edit-only, standard, RL, RW, DPO, and SW 
...
Options:
  ...
  --output PATH                 Output base directory (default: evaluation_results/normal/text_parallel_cot_8b_trajectory)
  ...
  --standard-text-checkpoint P  Standard Text checkpoint (default: checkpoints/normal/cot_8b_trajectory/standard/final)
  --rl-text-checkpoint P        RL Text checkpoint (default: checkpoints/normal/cot_8b_trajectory/rl/final)
  --rw-text-checkpoint P        RW Text checkpoint (default: checkpoints/normal/cot_8b_trajectory/rw/final)
  --dpo-text-checkpoint P       DPO Text checkpoint (default: checkpoints/normal/cot_8b_trajectory/dpo/final)
  --sw-text-checkpoint P        SW Text checkpoint (default: checkpoints/normal/cot_8b_trajectory/sw/final)
...
Examples:
  ...
  bash scripts/evaluation/start_eval_all_planner_text_trajectory_8b.sh

  # Evaluate custom number of samples on GPUs 3-9
  bash scripts/evaluation/start_eval_all_planner_text_trajectory_8b.sh --num-samples 100 --baseline-gpu 3 --edit-only-gpu 4 --standard-text-gpu 5 --rl-text-gpu 6 --rw-text-gpu 7 --dpo-text-gpu 8 --sw-text-gpu 9
EOF
            exit 0
            ;;
        *)
...
echo "════════════════════════════════════════════════════════════════════════════════"
echo "  PARALLEL TRAJECTORY-BASED 8B TEXT-ONLY MODEL EVALUATION"
echo "════════════════════════════════════════════════════════════════════════════════"
echo ""
echo "Configuration:"
echo "  Data:        $DATA_PATH"
...
echo ""
echo "════════════════════════════════════════════════════════════════════════════════"
echo "  ✅ ALL TRAJECTORY-BASED 8B EVALUATIONS COMPLETE!"
echo "════════════════════════════════════════════════════════════════════════════════"
echo ""
echo "📂 Results:"
