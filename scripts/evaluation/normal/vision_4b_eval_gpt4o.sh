#!/bin/bash
# Evaluate GPT-4o planner on COMPLEX theme text-only trajectory samples
...
OUTPUT_BASE="$PROJECT_ROOT/evaluation_results/vision_parallel_complex_cot_4b_trajectory"
GPT4O_DIR="$OUTPUT_BASE/gpt4o"

# Action library (COMPLEX theme uses 20-action library)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            cat << EOF
Usage: bash scripts/evaluation/complex_theme/start_eval_gpt4o_planner_text.sh [OPTIONS]

Evaluate GPT-4o Vision planner on COMPLEX theme text-only trajectory samples.

Options:
  --data PATH                   Path to evaluation data (default: training_data/complex_cot_8b_trajectory/full_dataset_for_eval.json)
  --output DIR                  Output directory (default: evaluation_results/text_parallel_complex_cot_8b_trajectory/gpt4o)
  --sample-ids-file FILE        File with sample IDs (default: training_data/complex_cot_8b_trajectory/test_samples_complex_cot_8b.txt)
...
Examples:
  ...
  bash scripts/evaluation/complex_theme/start_eval_gpt4o_planner_text.sh

  # Planner-only (5 samples, no images)
  bash scripts/evaluation/complex_theme/start_eval_gpt4o_planner_text.sh --num-samples 5 --no-images

  # Full evaluation (10 samples with images)
  bash scripts/evaluation/complex_theme/start_eval_gpt4o_planner_text.sh --num-samples 10
EOF
            exit 0
            ;;
        *)
...
echo "════════════════════════════════════════════════════════════════════════════════"
echo "  GPT-4O PLANNER EVALUATION (COMPLEX THEME)"
echo "════════════════════════════════════════════════════════════════════════════════"
echo ""
echo "Configuration:"
echo "  Theme:          COMPLEX (interior design)"
echo "  Data:           $DATA_PATH"
...
TEMP_SAMPLE_IDS="/tmp/gpt4o_eval_complex_samples_$$.txt"
head -$NUM_SAMPLES "$TEST_SAMPLES_FILE" > "$TEMP_SAMPLE_IDS"

echo "Sample IDs (first $NUM_SAMPLES from complex theme):"
...
echo "════════════════════════════════════════════════════════════════════════════════"
echo "  Starting GPT-4o evaluation on complex theme..."
echo "════════════════════════════════════════════════════════════════════════════════"
echo ""
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "  ✅ GPT-4O EVALUATION COMPLETE! (COMPLEX THEME)"
    echo "════════════════════════════════════════════════════════════════════════════════"
    echo ""
    ...
    echo ""
    echo "Note: These are COMPLEX theme interior design samples"
    echo ""
else
