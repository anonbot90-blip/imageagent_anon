#!/bin/bash
# Parallel evaluation for TRAJECTORY-BASED 8B baseline, standard, RL, RW, and DPO vision models
# VISION TRAJECTORY EXPERIMENT: Runs 8 models in PARALLEL (7 on GPUs + 1 via API)
# Ensures ALL models evaluate the SAME samples for fair comparison

# NOTE: set -e removed — consolidate_vision_results.sh is broken and non-fatal.
# Do NOT add set -u either — PYTHON_BIN is not defined (bash defaults to `python` in PATH).
...
OUTPUT_BASE="$PROJECT_ROOT/evaluation_results/normal/vision_parallel_cot_8b_trajectory"
NUM_SAMPLES=199  # Evaluate on 200 low-quality test samples
...
STANDARD_VISION_CHECKPOINT="$PROJECT_ROOT/checkpoints/normal/cot_8b_trajectory/vision/standard/final"
RL_VISION_CHECKPOINT="$PROJECT_ROOT/checkpoints/normal/cot_8b_trajectory/vision/rl/final"
RW_VISION_CHECKPOINT="$PROJECT_ROOT/checkpoints/normal/cot_8b_trajectory/vision/rw/final"
DPO_VISION_CHECKPOINT="$PROJECT_ROOT/checkpoints/normal/cot_8b_trajectory/vision/dpo/final"
SW_VISION_CHECKPOINT="$PROJECT_ROOT/checkpoints/normal/cot_8b_trajectory/vision/sw/final"
...
echo ""
echo "Models to evaluate: 8 (Baseline, Edit-Only, Standard, RL, RW, DPO, SW, GPT-4o Planner Vision)"
...
if [ $EVAL_CHECKPOINT -gt 0 ] && [ $EVAL_CHECKPOINT -le $NUM_SAMPLES ]; then
    ...
    CURRENT_SAMPLE=${START_SAMPLE:-0}
    BATCH_NUM=$((CURRENT_SAMPLE / EVAL_CHECKPOINT))
