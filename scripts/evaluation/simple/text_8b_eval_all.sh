#!/bin/bash
# Parallel evaluation for TRAJECTORY-BASED baseline, standard, RL, RW, and DPO text-only models
# TEXT-ONLY TRAJECTORY EXPERIMENT: Runs 5 models in PARALLEL on 5 different GPUs
...
RESULTS_DIR="$PROJECT_ROOT/imageagent_results_10000_cot_test"
DATA_PATH="$PROJECT_ROOT/training_data/cot_8b_trajectory/full_dataset_for_eval.json"
TEST_SAMPLES_FILE="$PROJECT_ROOT/training_data/simple/cot_8b_trajectory/test_samples_cot_8b.txt"
OUTPUT_BASE="$PROJECT_ROOT/evaluation_results/simple/text_parallel_cot_8b_trajectory"
...
# Model checkpoints - TRAJECTORY-BASED
BASELINE_CHECKPOINT="Qwen/Qwen3-VL-8B-Instruct"
EDIT_ONLY_CHECKPOINT="none"  # E baseline is inference-only, no checkpoint
STANDARD_TEXT_CHECKPOINT="$PROJECT_ROOT/checkpoints/simple/cot_8b_trajectory/standard/final"
RL_TEXT_CHECKPOINT="$PROJECT_ROOT/checkpoints/simple/cot_8b_trajectory/rl/final"
RW_TEXT_CHECKPOINT="$PROJECT_ROOT/checkpoints/simple/cot_8b_trajectory/rw/final"
DPO_TEXT_CHECKPOINT="$PROJECT_ROOT/checkpoints/simple/cot_8b_trajectory/dpo/final"
SW_TEXT_CHECKPOINT="$PROJECT_ROOT/checkpoints/simple/cot_8b_trajectory/sw/final"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            cat << EOF
Usage: bash scripts/evaluation/start_eval_all_planner_text_trajectory.sh [OPTIONS]

TRAJECTORY-BASED TEXT-ONLY EXPERIMENT (PARALLEL): Evaluates baseline, standard, RL, RW, and DPO 
text-only models trained with trajectory-based approach on the SAME samples using 5 GPUs in parallel.

Options:
  --data PATH                   Path to evaluation data (default: training_data/cot_8b_trajectory/full_dataset_for_eval.json)
  --output PATH                 Output base directory (default: evaluation_results/simple/text_parallel_cot_8b_trajectory)
  ...
  --baseline-checkpoint P       Baseline checkpoint (default: Qwen/Qwen3-VL-8B-Instruct)
  --standard-text-checkpoint P  Standard Text checkpoint (default: checkpoints/simple/cot_8b_trajectory/standard/final)
  --rl-text-checkpoint P        RL Text checkpoint (default: checkpoints/simple/cot_8b_trajectory/rl/final)
  --rw-text-checkpoint P        RW Text checkpoint (default: checkpoints/simple/cot_8b_trajectory/rw/final)
  --dpo-text-checkpoint P       DPO Text checkpoint (default: checkpoints/simple/cot_8b_trajectory/dpo/final)
...
Examples:
  # Evaluate test samples on GPUs 0-4 (parallel trajectory experiment)
  bash scripts/evaluation/start_eval_all_planner_text_trajectory.sh

  # Evaluate custom number of samples on GPUs 3-7
  bash scripts/evaluation/start_eval_all_planner_text_trajectory.sh --num-samples 100 --baseline-gpu 3 --standard-text-gpu 4 --rl-text-gpu 5 --rw-text-gpu 6 --dpo-text-gpu 7
EOF
            exit 0
            ;;
        *)
...
echo "════════════════════════════════════════════════════════════════════════════════"
echo "  PARALLEL TRAJECTORY-BASED TEXT-ONLY MODEL EVALUATION"
echo "════════════════════════════════════════════════════════════════════════════════"
echo ""
echo "Configuration:"
echo "  Data:        $DATA_PATH"
...
echo ""
echo "Checkpoints (TRAJECTORY-BASED):"
...
echo ""
echo "Models to evaluate: 5 (Baseline, Standard, RL, RW, DPO Text-Only Trajectory)"
...
# Create output directory and model directories
mkdir -p "$OUTPUT_BASE"
mkdir -p "$BASELINE_DIR" "$EDIT_ONLY_DIR" "$STANDARD_TEXT_DIR" "$RL_TEXT_DIR" "$RW_TEXT_DIR" "$DPO_TEXT_DIR" "$SW_TEXT_DIR" "$GPT4O_DIR" "$CONSOLIDATED_DIR"
    ...
    while [ $CURRENT_SAMPLE -lt $TOTAL_SAMPLES ]; do
        ...
        if [ $? -eq 0 ]; then
            ...
            if cp "$CONSOLIDATED_DIR"/*.png "$CONSOLIDATED_DIR"/*.json "$CONSOLIDATED_DIR"/*.md "$CHECKPOINT_DIR/" 2>&1; then
                ...
                echo "   ⚠️  Warning: Could not copy all files to checkpoint (some may be missing)"
            fi
        else
...
echo ""
echo "════════════════════════════════════════════════════════════════════════════════"
echo "  ✅ ALL TRAJECTORY-BASED EVALUATIONS COMPLETE!"
echo "════════════════════════════════════════════════════════════════════════════════"
echo ""
echo "📂 Results:"
...
echo ""
echo "Models evaluated (TRAJECTORY-BASED):"
echo "   1. Baseline (Qwen3-VL-8B-Instruct zero-shot)"
echo "   2. Standard Text-Only (trajectory-sampled training)"
echo "   3. RL Text-Only (trajectory-filtered training >= 3.0)"
echo "   4. RW Text-Only (trajectory-weighted training >= 3.5)"
echo "   5. DPO Text-Only (trajectory-preference pairs)"
echo ""
