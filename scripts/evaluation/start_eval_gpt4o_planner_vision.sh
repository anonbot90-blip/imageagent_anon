#!/bin/bash
# Evaluate GPT-4o planner on text-only trajectory samples
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            cat << EOF
Usage: bash scripts/evaluation/start_eval_gpt4o_planner_text.sh [OPTIONS]

Evaluate GPT-4o Vision planner on text-only trajectory samples.
...
Options:
  ...
  --sample-ids-file FILE        File with sample IDs (default: training_data/cot_8b_trajectory/test_samples_cot_8b.txt)
  ...
  --no-gpt-judge                Disable GPT-4o action judge
...
if [ "$NO_GPT_JUDGE" = "true" ]; then
    PYTHON_CMD="$PYTHON_CMD --no-gpt-judge"
fi
