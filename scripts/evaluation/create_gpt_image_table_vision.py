#!/usr/bin/env python3
"""
Create GPT-4o Image Quality comparison table from vision evaluation results.
...
def load_edit_only_scores(model_dir: Path) -> dict:
    """Load Edit-Only scores from sample metrics.json files (GPT-4o format)."""
    samples_dir = model_dir / "samples"
    if not samples_dir.exists():
        return None
    
    # Collect all GPT-4o scores from sample metrics.json files
    all_scores = []
    for sample_dir in sorted(samples_dir.iterdir()):
        ...
        # GPT-4o scores are at TOP LEVEL in metrics.json (not nested in image_metrics)
        gpt_scores = {
        ...
        if any(v and v > 0 for v in gpt_scores.values() if v is not None):
            all_scores.append(gpt_scores)
    
    if not all_scores:
        return None
    
    # Aggregate scores (only for non-None values)
    aggregated = {}
...
def create_gpt_image_table(model_dirs: dict, output_path: Path):
    ...
    for model_name, model_dir in model_dirs.items():
        # Use special loader for Edit-Only
        if model_name == "Edit-Only":
            scores = load_edit_only_scores(Path(model_dir))
        else:
    ...
    # Model order (Vision models - updated for 8 models)
    ...
    col_widths = [0.24, 0.07, 0.07, 0.07, 0.07, 0.08, 0.07, 0.07, 0.09, 0.11]  # Metric, B, E, S, R, RW, D, SW, GPT-4o, Winner
    ...
    # Add title
    plt.title('GPT-4o Image Quality Assessment (Vision Models)', 
              fontsize=14, weight='bold', pad=20)
    
    # Add caption matching the style of other tables
    caption = (
        "Legend: B=Baseline | E=Edit-Only (N/A for plan metrics) | S=Standard | R=RL | RW=RW | D=DPO | SW=SW | GPT-4o=GPT-4o | "
...
def main():
    parser = argparse.ArgumentParser(description="Create GPT-4o Image Quality comparison table for vision models")
