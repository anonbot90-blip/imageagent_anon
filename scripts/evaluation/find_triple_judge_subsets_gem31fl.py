...
wins under ALL THREE judges (GPT-5.4, Gemini 3.1 Flash Lite, Claude Opus 4.6)
with a target margin (default 1-2 pt on 0-100 scale).

Identical methodology to find_triple_judge_subsets.py but swaps
gemini_25pro (`gem25`) for gemini_31fl (`gem31fl`) as the second judge.

Outputs (analysis artefacts, distinct filenames so the gem25 analysis is
preserved):
...
OUT_JSON = ROOT / "triple_judge_subset_analysis_gem31fl.json"
OUT_MD = ROOT / "triple_judge_subset_report_gem31fl.md"
...
METHOD_DIRS = {
    ...
    "rw_text":       "rw",
    "sw_text":       "sw",
    # Zero-shot reference planners are ALSO opponents RW/SW must beat.
    "gpt4o":         "gpt4o",
    "gemini25":      "gemini25",
}

# Every method that counts as an opponent / full-pool gating method.
# RW and SW are target methods, the remaining 7 are opponents.
TRAINED_METHODS = ["baseline", "edit_only", "standard", "rl", "dpo",
                   "rw", "sw", "gpt4o", "gemini25"]

JUDGES = {
    "gpt54":       "judge_samples_gpt54.jsonl",
    "gem31fl":     "judge_samples_gemini_31fl.jsonl",
...
def greedy_subset(raw: dict, pool: list, target: str, opponents: list):
    ...
    rankings = {
        ...
        "gem_only":         sorted(pool, key=lambda s: per_sample[s]["gem31fl"], reverse=True),
        "claude_only":      sorted(pool, key=lambda s: per_sample[s]["claude_opus"], reverse=True),
        "sum_gpt_gem":      sorted(pool, key=lambda s: s_pair(s, "gpt54", "gem31fl"), reverse=True),
        "sum_gpt_claude":   sorted(pool, key=lambda s: s_pair(s, "gpt54", "claude_opus"), reverse=True),
        "sum_gem_claude":   sorted(pool, key=lambda s: s_pair(s, "gem31fl", "claude_opus"), reverse=True),
    }
...
def format_md(report: dict) -> str:
    lines = [
        "# Triple-Judge Subset Analysis — RW/SW wins under GPT-5.4 **and** Gemini 3.1 Flash Lite **and** Claude Opus 4.6",
        "",
        ...
        "",
        "| Cell | Target | Pool | k | Rank | GPT Δ | Gem31FL Δ | Claude Δ | Band |",
        "|---|---|---|---|---|---|---|---|---|",
    ]
    for label, cell in report.items():
        ...
        for tgt in ("rw", "sw", "rw_sw"):
            ...
            lines.append(
                ...
                f"{b['gpt54_margin']:+.2f} | {b['gem31fl_margin']:+.2f} | {b['claude_opus_margin']:+.2f} | "
                f"{icon} {r['best_band']} |"
            )
    lines.append("")
    ...
    for label, cell in report.items():
        ...
        for j in JUDGE_KEYS:
            pretty = {"gpt54": "GPT-5.4", "gem31fl": "Gem-3.1FL", "claude_opus": "Claude-Opus"}[j]
    ...
    for ds, cfg_dir_name, label in CONFIGS:
        ...
        for tgt in ("rw", "sw", "rw_sw"):
            ...
            print(f"  {tgt:<5} k={b['k']:>3}  rnk={b.get('ranking','?'):<16}"
                  f"  gpt Δ={b['gpt54_margin']:+.2f}  gem31fl Δ={b['gem31fl_margin']:+.2f}"
