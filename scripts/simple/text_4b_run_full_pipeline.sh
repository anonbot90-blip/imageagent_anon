...
# Full Text-Only Training Pipeline (Trajectory-Based) - SIMPLE Dataset 4B Model
...
print_header "Step 1/7: Verify Trajectory Split (SIMPLE 10K - 4B Model)"
...
print_header "📈 Evaluate Text-Only Models (SIMPLE 10K - 4B)"
...
# Use unified config-based evaluation for simple 4B
bash scripts/evaluation/run_parallel_evaluation.sh \
    --config scripts/evaluation/configs/simple_text_4b.yaml
...
print_header "🎉 SIMPLE Dataset 4B Training & Evaluation Complete!"
...
echo -e "${CYAN}   Evaluation: evaluation_results/simple/text_parallel_cot_4b_trajectory/${NC}"
echo ""

echo -e "${YELLOW}💡 Next steps:${NC}"
echo -e "${YELLOW}   - Review evaluation results${NC}"
echo -e "${YELLOW}   - Compare with 8B model (evaluation_results/simple/text_parallel_cot_8b_trajectory/)${NC}"
echo ""
